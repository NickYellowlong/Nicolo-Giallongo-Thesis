...
function WelcomeScreen() {
  ...
  const architecture = useGameState((state) => state.architecture);
  const setArchitecture = useGameState((state) => state.setArchitecture);
  return (      
        <>
          ...
          <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
            <button className='app_button' onClick={async() => {
              startLoading()
              const init = await api_caller.init(architecture);
...
function MotivationInsertion() {
  ...
  const architecture = useGameState((state) => state.architecture);
  const scene_counter = useGameState((state) => state.scene_counter);
  const increaseSceneCounter = useGameState((state) => state.increaseSceneCounter);
  const maxSceneCounter = useGameState((state) => state.maxSceneCounter);
  return (
    <div style={{display : 'flex', flexDirection : 'column', alignItems: 'center'}}>
      <textarea
        className='description-input'
      ...
      <>
        <button className='app_button' onClick={async () => {
          ...
          if (architecture) {
            ...
            narration = (JSON.parse(JSON.stringify(await GameManager.sendActionNoArchitecture("", maxSceneCounter, scene_counter)))).narration;
            increaseSceneCounter();
            handleStateChange(5);
          }
          stopLoading();
          }} style={{fontSize: '25px'}}>Continue</button>
      ...
      <>
        <button className='app_button' onClick={async () => {
          ...
          handleStateChange(narration_return.end? 6: (narration_return.change_act? 4 : 5));
            ...
            startLoading();
            console.log("No architecture")
            narration = (JSON.parse(JSON.stringify(await GameManager.sendActionNoArchitecture(action, maxSceneCounter, scene_counter)))).narration;
            increaseSceneCounter();
            if (useGameState.getState().scene_counter <= maxSceneCounter) {
            handleStateChange(5);
            }
          else {
            handleStateChange(6);
