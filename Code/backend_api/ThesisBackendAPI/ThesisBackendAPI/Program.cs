...
app.MapPost("/narration_no_architecture", async (NoArchitectureRequest request) =>
{
    string content = request.content;
    int maxScenes = request.maxScenes;
    int currentScene = request.currentScene;
    string prompt = Narrator.BuildPromptForNoArchitecture(content, maxScenes, currentScene);
    GeneratorResponse result = await APICaller.GenerateNoArchitecture(prompt);
    SceneInfo sceneInfo = new SceneInfo();
    sceneInfo.user_input = content;
    sceneInfo.prompt = prompt;
    sceneInfo.generation_ms = result.processing_time_ms;
    sceneInfo.generated_text = result.content.narration;
    InformationForNoArchitecture.storySoFar += "\n" + result.content.narration;
    AppData.test.scenes.Add(sceneInfo);
    AppData.SaveTestData();
    return Results.Ok(new
        {
...
}).WithName("PostNarrationNoArchitecture");
