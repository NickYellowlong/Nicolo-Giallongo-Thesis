...
public class Narrator
{
    ...
    public static string BuildPromptForNoArchitecture(string content, int maxScenes, int currentScene)
    {
        string prompt = "";
        prompt += "STORY OUTLINE: \n\n";
        prompt += InformationForNoArchitecture.storyOutline;
        prompt += "\n\n\n";
        prompt += BuildCharacterInformation();
        prompt += "\n\n\n";
        if (!string.IsNullOrEmpty(InformationForNoArchitecture.storySoFar)){}
            prompt += "STORY SO FAR:\n\n" + InformationForNoArchitecture.storySoFar + "\n\n\n";
            
        if (content != null && content != "")
            prompt += "The user wants to perform the following actions:\n" + content + "\n\n\n";

        prompt += $"This is the scene {currentScene}/{maxScenes}\n";
        prompt +=
            "\n\nRemember:\nreply with only the story, no additional elements;\nalways narrate in third person;\nnever address the user directly.\nnever comment the story in any way";
