...
no_architecture_system_info = requests_info.get("generate_narration_no_architecture")
if not no_architecture_system_info:
    logger.warning("generate_narration_no_architecture prompt not found in requests_information.json; using fallback from config if available")
    no_architecture_system_info = getattr(config, "NO_ARCHITECTURE_SYSTEM_PROMPT", None)

if no_architecture_system_info:
    text_generator.set_no_architecture_system_info(no_architecture_system_info)
else:
    logger.error("No system prompt set for no-architecture generator. Generation requests may fail.")
